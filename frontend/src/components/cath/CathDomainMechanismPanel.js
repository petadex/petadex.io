...
import { mcsaEntryUrl } from "../../utils/cathDomainSectionConfig"
...
const CathDomainMechanismPanel = ({ mechanism, references }) => {
          ...
          {showMore &&
            mechanism.customSummary.split("\n\n").map((para, i) => (
              <p key={i} className="text-sm text-muted-foreground leading-relaxed mb-4">
                {renderTextWithCitations(para, references, `mechanism-${i}`)}
              </p>
            ))}
        </div>
      )}
...
export default CathDomainMechanismPanel
